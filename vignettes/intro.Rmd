...
Open access copies of scholarly publications are sometimes hard to find. Some were made freely available as preprint before publication, others were deposited in an institutional repository, a digital archive maintained by universities and research institutions. This document guides you to roadoi, a R client that makes it easy to search for these open access copies by interfacing the [oaDOI.org](https://oadoi.org/) where DOIs are matched with full-text links in open access journals and archives.
...
- [Datacite](https://www.datacite.org/): another DOI registration agency with the main focus on research data
- [Directory of Open Access Journals (DOAJ)](https://doaj.org/): a comprehensive registry of open access journals
...
* `is_boai_license`: TRUE whenever the license is cc-by, cc0, or PD. This is the highly-regarded [BOAI definition of Open access](http://www.budapestopenaccessinitiative.org/)
...
DOIs have become essential to reference scholarly publications, and thus many digital libraries and institutional databases now keep track of these persistent identifiers. For the sake of this vignette, instead of starting with a pre-defined set of publications originating from these sources, we simply generate a random sample of 200 DOIs registered with Crossref by using the [rcrossref package](https://github.com/ropensci/rcrossref).

```{r, message=FALSE}
library(dplyr)
# install rcrossref from CRAN
...
After gathering the data, reporting with R is very straightforward. In combination with [R Markdown](http://rmarkdown.rstudio.com/) and related packages you can even generate dynamic reports, thus making your study reproducible and transparent for others.
